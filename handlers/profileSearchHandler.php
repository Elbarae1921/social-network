<?php 
    ...
    else
    {
        ...
        $query = mysqli_query($con, "SELECT id, title, post, likes, writer FROM posts WHERE writer = '$writer' AND (title like '%$val%' or post like '%$val%' or genre like '%$val%' or (SELECT username FROM users WHERE id = writer) like '%$val%')");
